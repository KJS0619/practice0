# 매월 1일에 실행되는 블로그 월간 다이제스트 스크립트 (Windows 작업 스케줄러 "BlogMonthlyDigest"가 호출)
# 지난 달 한 달간 blog/posts/에 쌓인 글들을 모아 blog/digests/monthly/<지난달 YYYY-MM>/ 에 다이제스트를 만든다.

[Console]::OutputEncoding = [System.Text.Encoding]::UTF8
$OutputEncoding = [System.Text.Encoding]::UTF8
chcp 65001 > $null

# 매일 트리거되지만(작업 스케줄러에 Monthly 트리거가 없어서), 매월 1일에만 실제로 동작한다.
if ((Get-Date).Day -ne 1) {
  exit 0
}

$today = Get-Date -Format "yyyy-MM-dd"
$lastMonthDate = (Get-Date).AddMonths(-1)
$targetMonth = $lastMonthDate.ToString("yyyy-MM")
$startDate = (Get-Date -Year $lastMonthDate.Year -Month $lastMonthDate.Month -Day 1).ToString("yyyy-MM-dd")
$endDate = (Get-Date -Year $lastMonthDate.Year -Month $lastMonthDate.Month -Day ([DateTime]::DaysInMonth($lastMonthDate.Year, $lastMonthDate.Month))).ToString("yyyy-MM-dd")

$logDir = "D:\workspace\practice0\blog\digests\_logs"
New-Item -ItemType Directory -Force -Path $logDir | Out-Null
$logFile = Join-Path $logDir "monthly_$today.log"
$digestDir = "D:\workspace\practice0\blog\digests\monthly\$targetMonth"
...
오늘 날짜는 $today 다. 이번 월간 다이제스트 대상 기간은 $targetMonth 한 달 전체($startDate ~ $endDate) 다. 출력 폴더는 $digestDir 이고, 이미 만들어져 있다.

1. 다이제스트 라이터 서브에이전트를 호출해라. D:\workspace\practice0\blog\agent\digest.md 지침을 따르되, D:\workspace\practice0\blog\posts\ 아래에서 $startDate 부터 $endDate 사이 날짜 폴더들을 찾아 그 안의 draft_final.md 를 전부 자료로 사용하게 해라(폴더가 없는 날짜는 건너뛴다). guide/style-guide.md, guide/seo-guide.md 도 반드시 읽게 해라. 결과를 $digestDir\draft.md 로 저장하게 해라(이미지는 <!-- IMAGE: [설명] --> 마커로 2~4곳).
...
"===== $today 월간 다이제스트 실행 시작 (대상: $targetMonth, $startDate ~ $endDate) =====" | Out-File -FilePath $logFile -Append -Encoding utf8
...
if (Test-Path (Join-Path $digestDir "draft_final.pdf")) {
  ...
  try {
    node send-digest-mail.js $digestDir "월간" 2>&1 | Out-File -FilePath $logFile -Append -Encoding utf8
  } catch {
    "메일 발송 오류: $($_.Exception.Message)" | Out-File -FilePath $logFile -Append -Encoding utf8
  }
  try {
    node send-digest-telegram.js $digestDir "월간" 2>&1 | Out-File -FilePath $logFile -Append -Encoding utf8
  } catch {
...
"===== $today 월간 다이제스트 실행 종료 =====" | Out-File -FilePath $logFile -Append -Encoding utf8
